...
class aQuery:
    ...
    def answerQuery(self):
        ...
        listaCabecalho.append("MESSAGE-ID = " + str(self.message_id) + ",FLAGS = " +
                              self.flags + ",RESPONSE-CODE = " + str(self.response_code))
        ...
        for it3 in listaExtraValues:
            ...
            if (self.typeValue.lower() in listaP[0] or "ns" in listaP[0]):
                ...
                nExtraValues += 1
        listaCabecalho.append("N-VALUES = "+str(nValues)+" , "+"N-AUTHORITIES =" +
                              str(nAuth)+" , "+"N-EXTRA-VALUES ="+str(nExtraValues)+" ;")
        listaCabecalho.append("# Data: Query Info")
        listaCabecalho.append("QUERY-INFO.NAME = "+" @ ," +
                              " QUERY-INFO.TYPE = "+self.typeValue+" ;")
        listaCabecalho.append(
            "# Data: List of Response, Authorities and Extra Values")

        m = MessageDNS(self.message_id, self.flags, self.response_code, nValues, nAuth, nExtraValues,
                       self.domain, self.typeValue, guardaResponseValues, guardaAuthValues, guardaExtraValues)
        bytes = m.serialize()
