...
class cl:
    ...
    def runCL(self):
        ...
        m = str(message_id) + flags + str(response_code) + str(numberOfValues) + \
            str(numberOfAuthorities) + str(numberOfExtraValues) + domain + type
        ...
        msg = MessageDNS(message_id, flags, response_code, numberOfValues, numberOfAuthorities,
                         numberOfExtraValues, domain, type, responseValues, authoritiesValues, extraValues)
        ...
        now = datetime.today().isoformat()
        writeLogFile = logF(str(now), "QR/QE",
                            "localHost:"+str(3333), m, self.logF)
        writeLogFile.escritaLogFile()
        ...
        now = datetime.today().isoformat()
        writeLogFile = logF(str(now), "RP/RR",
                            "localHost:"+str(3333), m, self.logF)
        writeLogFile.escritaLogFile()
