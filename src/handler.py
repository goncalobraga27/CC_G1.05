...
class hd:
    ...
    def perguntaAoSeuSP(self):
        ...
        now = datetime.now()
        writeLogFile = logF(str(now), "QR/QE", self.ipSR+":"+str(self.portaSR),
                            msg_UDP.decode('utf-8'), self.listaLogFile[0])
        writeLogFile.escritaLogFile()
        sck = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        pedido = self.proQuery.typeValue.encode('UTF-8')
        sck.sendto(pedido, (self.listaIP_SP[0], 3332))
        msg_UDP, _ = sck.recvfrom(1024)
        ...
        for i in range(0, numberLinhas+1):
            ...
            if (len(listaParametrosLinha) == 6):
                e1 = entry(self.domain, self.proQuery.typeValue,
                           listaParametrosLinha[3], ttl, listaParametrosLinha[5], "SP", datetime.now(), "0", "VALID")
                self.c.addEntry(e1)
            if (len(listaParametrosLinha) == 5):
                e1 = entry(self.domain, self.proQuery.typeValue,
                           listaParametrosLinha[3], ttl, "0", "SP", datetime.now(), "0", "VALID")
                self.c.addEntry(e1)

        ansQuerySR = aQuerySR(self.proQuery.message_id, "R", str(
            2), self.c.cache, self.proQuery.typeValue, self.domain)
    ...
    def perguntaLEIandSRLEI(self):
        ...
        sckSDT.sendto(self.domain.encode('UTF-8'),
                      (listaParSDT[0], int(listaParSDT[1])))
        msg, _ = sckSDT.recvfrom(1024)
