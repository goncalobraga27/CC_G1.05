    ...
    def runSR(self):
        ...
        if self.listaIP_SP == [] and self.listaPorta_SP == [] and self.listaLogFile == []:
            ...
            writeLogFile = logF(str(now), "FL", "127.0.0.1",
                                "Parse config File error", self.listaLogFile[0])
        ...
        while True:
            ...
            ansQuerySR = aQuerySR(proQuery.message_id, "R", str(
                0), c.cache, proQuery.typeValue, domain)
            if (queryCheck == 1):
                sys.stdout.write("\nA query pedida não é válida\n")
            if (queryCheck == 0 and ansQuerySR.canGiveResponse() == True):
                _, cod_message = ansQuerySR.answerQuerySR()
                sck_UDP.sendto(cod_message, add_UDP)

            else:
                if (queryCheck == 0 and domain == self.domainSR):
                    handler = hd(proQuery, add_UDP, domain,
                                 c, sck_UDP, msg_UDP)
